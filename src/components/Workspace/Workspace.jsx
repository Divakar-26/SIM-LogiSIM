...
// Base: document-5 feature set (ghost placement, regions, copy/paste, waypoints,
//       junction insertion, clock manager, bit toggle, all shortcuts)
// Perf: cameraRef DOM trick (zero re-renders during pan), useMemo nodeMap/selectedSet/
//       nodeColors, useCallback handlers, setMousePos only when drawing wire,
//       pass nodeColor+cameraRef to Node so nodes don't re-render on camera change.
...
// ── Ghost node (for pending placement) ───────────────────────────────────────
function GhostNode({ type, x, y, width, height }) {
    ...
    return (
        <div style={{
            ...
            background: bg, border:"2px dashed rgba(137,180,250,0.7)",
...
function Workspace({ nodes, setNodes, wires, setWires, regions, setRegions, pendingTypes, onPlacePending, onCancelPending }) {
    ...
    const cameraLayerRef = useRef(null);           // direct DOM for zero-rerender pan
    const cameraRef      = useRef({x:0,y:0,zoom:1}); // always-fresh camera
    ...
    // Pre-compute node colors so Node doesn't need useSettings context subscription
    ...
    // ── Propagation with signal fingerprint (skip on position/label changes) ──
    ...
    useEffect(() => {
        ...
        const merged = nodes.map(orig => {
            ...
            anyChanged=true; return n;
        });
        if (anyChanged) { prevSigRef.current=''; setNodes(merged); }
    ...
    const applyCameraDOM = (cam) => {
        ...
        if (cameraLayerRef.current)
            cameraLayerRef.current.style.transform = `translate(${cam.x}px,${cam.y}px) scale(${cam.zoom})`;
    };
    ...
    const onSelectNode = useCallback((id) => {
        setSelectionBox(null); setSelectedNodes([id]);
    }, []);
    ...
    useEffect(() => {
        const onKey=(e)=>{
            ...
            if (e.key==="Escape") {
                cancelWire(); setNodeMenu(null); setClockConfig(null); setTool("select");
                onCancelPending?.(); setGhostWorldPos(null); setRegionPrompt(null);
            }
            ...
            if (mod&&e.key==="g") {
                e.preventDefault();
                if (selectedRef.current.length>=2) setRegionPrompt({nodeIds:[...selectedRef.current],x:window.innerWidth/2,y:window.innerHeight/2});
            }
        };
        window.addEventListener("keydown",onKey);
        return ()=>window.removeEventListener("keydown",onKey);
    }, [handleDeleteSelected,cancelWire,fitAll,focusOrigin,handleCopy,handlePaste]);
    ...
    const handleMouseMove=(e)=>{
        ...
        if (isPanningRef.current) {
            ...
            applyCameraDOM(cam);  // direct DOM — zero React re-renders during pan
            return;
        }
        if (selectionBox) setSelectionBox(prev=>prev?{...prev,endX:sx,endY:sy}:null);
        // Only update mousePos when drawing a wire (huge perf win)
    ...
    const handleMouseUp=()=>{
        if (isPanningRef.current) {
            ...
            setCamera({...cameraRef.current}); // sync React state once, on pan end
            return;
        }
        if (selectionBox) {
            ...
            >
                <div ref={cameraLayerRef} className="camera-layer" style={{
                    ...
                    <svg className="wire-layer" style={{pointerEvents:"none"}}>
                        ...
                        {/* Junction dots at multi-driven output pins */}
                        {(()=>{
                            const srcMap={};
                            wires.forEach(w=>{
